...
export function createPrismaStore(prisma) {
  const db = {
    ...
    async entryUpdate(userId, id, input) {
      const existing = await prisma.foodEntry.findFirst({ where: { id: Number(id), userId: Number(userId) } });
    ...
    async weightUpdate(userId, id, input) {
      const existing = await prisma.weightEntry.findFirst({ where: { id: Number(id), userId: Number(userId) } });
      if (!existing) return null;
      const data = {};
    ...
    async aiCacheDelete(userId) {
      await prisma.aiAnalysisCache.deleteMany({ where: { userId: Number(userId) } });
    }
  };
...
export async function initPrismaStore() {
  ...
  const prismaUrl = process.env.DATABASE_URL || config.db.url;
  const prisma = new PrismaClient({ datasourceUrl: prismaUrl });
  try {
    await prisma.$queryRaw`SELECT 1`;
  } catch (error) {
    await prisma.$disconnect();
    throw new Error(`Cannot connect to database at ${prismaUrl} (${error.message}). Check DATABASE_URL env var.`);
  }
  return createPrismaStore(prisma);
}
